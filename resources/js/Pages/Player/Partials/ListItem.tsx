...
export default function ListItem({
    index,
    player,
}: PropsWithChildren<{ index: number; player: iPlayer }>) {
    ...
    function setRating(rate: number) {
        ...
        data.rating = String(rate);

        post(route('players.rate'), {
            preserveScroll: true,
            onSuccess: (s) => {},
            onError: (errors) => {},
        });
    ...
    function deleteMe() {
        if (!confirm('Are you sure? Do you want to delete this player?')) {
            return false;
        }
        console.log('delete', player.id);

        post(route('players.delete', player.id), {
            preserveScroll: true,
            onSuccess: (s) => {
                // Handle success
            ...
            onError: (errors) => {
                // Handle error
                console.error('Error deleting rating', errors);
            },
        });
    }
    ...
    return (
        <tr key={index}>
            <td className="px-10 py-4">#{player.id}</td>
